...
export default function SignUp() {
    ...
    const handleChangePassword = async (e) => {
      ...
      await setPassword_v(regExp.test(e.target.value));
      if(!password_v){
        await setPasswordMessage("유효한 비밀번호 형식이 아닙니다.");
      }
      else {
          ...
          <div className="bg-gray-300">
            <div className="h-max container max-w-2xl mx-auto flex-1 flex flex-col items-center justify-center px-2 py-3">
              ...
              <div className="bg-white px-10 py-8 rounded shadow-lg text-black w-full">
                ...
                <p className="ml-1 text-slate-900 text-sm">* 입력하신 주소로 인증메일이 발송됩니다. 사용 가능한 주소를 입력해 주세요.</p>
                <input
                  type="text"
                  className="border border-gray-400 w-full p-3 rounded"
                  name="이메일"
                ...
                />
                <p className="ml-2 mb-4 text-red-500 text-sm mb-4">{passwordMessage}</p>
                ...
                <button
                  type="submit"
                  className="w-full text-center mt-5 py-3 rounded-2xl text-lg bg-slate-700 text-white hover:bg-white hover:text-gray-900 focus:outline-none my-1 font-medium transition ease-in-out shadow-md"
                >
                  계정 생성
                </button>
